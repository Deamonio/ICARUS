...
def plot_metrics_callback(trainer):
    """매 에포크마다 실행되는 콜백 함수"""
    global graph_folder, train_dir, epoch_start_time, metrics_log_file
    ...
    if graph_folder is None:
        ...
        # 메트릭 로그 파일 생성 및 헤더 작성
        metrics_log_file = os.path.join(graph_folder, 'epoch_metrics.csv')
        with open(metrics_log_file, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['Epoch', 'Duration(sec)', 'Duration(min)', 'Precision', 'Recall', 'mAP50', 'mAP50-95', 'F1_Score'])
        print(f"메트릭 로그 파일 생성: {metrics_log_file}")
    ...
    try:
        ...
        if hasattr(trainer, 'metrics') and trainer.metrics:
            ...
            # 메트릭 값 추출
            precision = metrics.get('metrics/precision(P)', 0.0)
            recall = metrics.get('metrics/recall(P)', 0.0)
            mAP50 = metrics.get('metrics/mAP50(P)', 0.0)
            mAP50_95 = metrics.get('metrics/mAP50-95(P)', 0.0)
            ...
            if metrics_log_file is not None:
                with open(metrics_log_file, 'a', newline='', encoding='utf-8') as f:
                    writer = csv.writer(f)
                    writer.writerow([
                        current_epoch,
                        f"{epoch_duration:.2f}",
                        f"{epoch_duration/60:.2f}",
                        f"{precision:.4f}",
                        f"{recall:.4f}",
                        f"{mAP50:.4f}",
                        f"{mAP50_95:.4f}",
