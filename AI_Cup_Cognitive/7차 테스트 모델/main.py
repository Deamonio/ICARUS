...
def calculate_metrics(model, dataloader, device, img_size=800, use_tta=False):
    ...
    with torch.no_grad():
        for images, boxes, points in dataloader:
            ...
            # 통합 Loss (Box 1배, Point 10배 가중치)
            loss = (1.0 * box_loss) + (10.0 * point_loss)
...
def main():
    ...
    # [차이점] Backbone 고정 코드를 삭제했습니다. 이제 전체를 학습합니다.
    print("\n🚀 전체 가중치 학습 모드 시작 (Backbone Unlocked)")
    ...
    # 처음부터 학습하므로 러닝레이트를 약간 높게(1e-4) 시작하여 깎아나갑니다.
    ...
    graph_filename = f'training_metrics_{timestamp}.png'
    history = []
    start_epoch = 0
    ...
    patience_counter = 0
    best_model_path = 'cup_model_best.pth'
    latest_model_path = 'cup_model_latest.pth'
    epochs = 150  # 150 에포크 목표

    # ========== 학습 루프 (Early Stopping 포함) ==========
    print("\n" + "=" * 100)
    print("🔥 전체 학습 시작! (Resolution: 800x800, Batch Size: 8, Epochs: 150, LR: 0.0001)")
