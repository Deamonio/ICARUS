...
class CupDataset(Dataset):
    def __init__(self, img_dir, label_dir, img_size=800, augment=False):
        ...
        if self.augment:
            self.transform = A.Compose([
                ...
                # [최종 튜닝] 미세 흙들림만 대응, 중심점 고정 집중
                A.ShiftScaleRotate(shift_limit=0.05, scale_limit=0.05, rotate_limit=15, p=0.5),
                
                A.HorizontalFlip(p=0.5),
                A.RandomBrightnessContrast(p=0.3),  # 조명 변화도 완화
            ...
            self.transform = A.Compose([
                # 1. 가로세로 비율 유지하며 긴 쪽을 640으로 맞춤
                A.LongestMaxSize(max_size=img_size),
                # 2. 남는 공간을 회색(114, 114, 114)으로 채워 640x640 정규화
                A.PadIfNeeded(min_height=img_size, min_width=img_size, border_mode=0, value=(114, 114, 114)),
                A.Normalize(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)),
                ToTensorV2()
            ], keypoint_params=A.KeypointParams(format='xy', remove_invisible=False),
    ...
    def __getitem__(self, idx):
        ...
        # 강한 증강으로 bbox나 keypoint가 사라진 경우 기본 변환 사용
        if len(transformed['bboxes']) == 0 or len(transformed['keypoints']) == 0:
            transformed = self.basic_transform(image=image, bboxes=[bbox], class_labels=[0], keypoints=keypoint)
...
def calculate_metrics(model, dataloader, device, img_size=800, use_tta=False):
    ...
    with torch.no_grad():
        for images, boxes, points in dataloader:
            ...
            # 통합 Loss (Box 5배, Point 80배 - 최종 튜닝 단계)
            loss = (5.0 * box_loss) + (80.0 * point_loss)
...
def main():
    ...
    # ========== 데이터셋 로드 ==========
    print("\n📦 데이터셋 로드 중...")
    train_dataset = CupDataset('./dataset/images/train', './dataset/labels/train', img_size=800, augment=True)
    ...
    # Stage 1.5 전이학습 가중치 로드
    if os.path.exists('cup_model_transfer_best.pth'):
    ...
    csv_filename = f'transfer2_metrics_{timestamp}.csv'
    graph_filename = f'transfer2_metrics_{timestamp}.png'
    history = []
    start_epoch = 0
    
    print(f"\n📝 전이학습 Stage 2 세션 시작 (최종 튜닝)")
    ...
    patience_counter = 0
    best_model_path = 'cup_model_transfer2_best.pth'
    latest_model_path = 'cup_model_transfer2_latest.pth'
    epochs = 80  # 80 에포크 목표 (최종 튜닝)

    # ========== 학습 루프 (Early Stopping 포함) ==========
    print("\n" + "=" * 100)
    print("🔥 전이학습 Stage 2 시작! (최종 튜닝 - Resolution: 800x800, Batch: 8, Epochs: 80, LR: 1e-5)")
    print("  🎯 수렴 모드: 15° 회전 + 스케일 0.05 (중심점 고정 집중)")
    print("  💯 손실 가중치: Box 5x + Point 80x (폭발적 상향)")
