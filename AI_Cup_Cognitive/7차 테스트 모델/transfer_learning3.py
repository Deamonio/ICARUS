...
def calculate_metrics(model, dataloader, device, img_size=800, use_tta=False):
    ...
    with torch.no_grad():
        for images, boxes, points in dataloader:
            ...
            # Box Loss 계산 (강화: 2.0 * L1 + MSE)
            box_loss = (2.0 * criterion_l1(p_boxes, boxes)) + criterion_mse(p_boxes, boxes)
            point_loss = criterion_mse(p_points, points)
            total_box_loss += box_loss.item()
            
            # 통합 Loss (Box 40배, Point 60배 - Stage 3.5 리밸런싱)
            loss = (40.0 * box_loss) + (60.0 * point_loss)
...
def plot_training_metrics(history, save_path='training_metrics.png', best_epoch=None):
    ...
    fig.suptitle('Transfer Learning - Box & Center Point Training Metrics (with Early Stopping)', fontsize=16, fontweight='bold')
...
def main():
    ...
    # Stage 3.5: 최신 가중치 로드 (우선순위 순서)
    if os.path.exists('cup_model_transfer3_best.pth'):
        model.load_state_dict(torch.load('cup_model_transfer3_best.pth', map_location=device))
        print("  ✅ Stage 3 가중치 로드 완료: cup_model_transfer3_best.pth")
    elif os.path.exists('cup_model_transfer2_best.pth'):
        model.load_state_dict(torch.load('cup_model_transfer2_best.pth', map_location=device))
        print("  ✅ Stage 2 가중치 로드 완료: cup_model_transfer2_best.pth")
    elif os.path.exists('cup_model_transfer_best.pth'):
        model.load_state_dict(torch.load('cup_model_transfer_best.pth', map_location=device))
        print("  ✅ Stage 1.5 가중치 로드 완료: cup_model_transfer_best.pth")
    elif os.path.exists('cup_model_best.pth'):
        model.load_state_dict(torch.load('cup_model_best.pth', map_location=device))
        print("  ✅ 기본 가중치 로드 완료: cup_model_best.pth")
    else:
        print("  ⚠️  가중치 파일을 찾을 수 없습니다. 처음부터 학습합니다.")
    
    print("  ✅ YOLOFullTaskModel (Deep Multi-Head with BatchNorm)")

    # Stage 3.5 긴급 재튜닝: Local Minima 탈출을 위한 학습률 상향
    optimizer = torch.optim.Adam(model.parameters(), lr=0.00005)  # 5e-5 (10배 상향)
    ...
    csv_filename = f'transfer3.5_metrics_{timestamp}.csv'
    graph_filename = f'transfer3.5_metrics_{timestamp}.png'
    history = []
    start_epoch = 0
    
    print(f"\n📝 전이학습 Stage 3.5 세션 시작 (긴급 재튜닝 - Local Minima 탈출)")
    ...
    patience_counter = 0
    best_model_path = 'cup_model_transfer3.5_best.pth'
    latest_model_path = 'cup_model_transfer3.5_latest.pth'
    epochs = 100  # 100 에포크 목표 (Stage 3.5)

    # ========== 학습 루프 (Early Stopping 포함) ==========
    print("\n" + "=" * 100)
    print("🔥 전이학습 Stage 3.5 시작! (긴급 재튜닝 - Resolution: 800x800, Batch: 8, Epochs: 100, LR: 5e-5)")
    print("  🚫 증강 완전 제거: 원본 데이터에만 100% 최적화")
    print("  ⚡ 손실 가중치 리밸런싱: Box 40x + Point 60x (박스 힘 강화)")
    print("  🚀 학습률 10배 상향: Local Minima 탈출 모드")
