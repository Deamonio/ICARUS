    ...
    def __init__(self, img_dir, label_dir, img_size=800, augment=False):
        ...
        if self.augment:
            self.transform = A.Compose([
                ...
                # [핵심] 각도를 45도까지 과감하게 틀고, 스케일 변화도 줍니다.
                A.ShiftScaleRotate(shift_limit=0.1, scale_limit=0.2, rotate_limit=45, p=0.8),
                
                # [추가] 컵이 화면 구석에서 왜곡되는 현상 대비 (입체감 확보)
                A.Perspective(scale=(0.05, 0.1), p=0.5),
                
                A.HorizontalFlip(p=0.5),
                A.RandomBrightnessContrast(p=0.4),  # 조명 변화 대응 강화
...
def calculate_metrics(model, dataloader, device, img_size=800, use_tta=False):
    ...
    with torch.no_grad():
        for images, boxes, points in dataloader:
            ...
            # 통합 Loss (Box 2배, Point 20배 가중치 - 각도 변화 대응)
            loss = (2.0 * box_loss) + (20.0 * point_loss)
...
def main():
    ...
    if os.path.exists('cup_model_best.pth'):
        ...
        print("  ✅ Best 가중치 로드 완료: cup_model_best.pth")
    else:
        print("  ⚠️  Best 가중치 파일을 찾을 수 없습니다. 처음부터 학습합니다.")
    
    print("  ✅ YOLOFullTaskModel (Deep Multi-Head with BatchNorm)")

    # 전이학습용 학습률 (기존 지식 보존하면서 새로운 각도 적응)
    ...
    csv_filename = f'transfer_learning_metrics_{timestamp}.csv'
    graph_filename = f'transfer_learning_metrics_{timestamp}.png'
    history = []
    start_epoch = 0
    ...
    patience_counter = 0
    best_model_path = 'cup_model_transfer_best.pth'
    latest_model_path = 'cup_model_transfer_latest.pth'
    epochs = 80  # 80 에포크 목표 (전이학습)

    # ========== 학습 루프 (Early Stopping 포함) ==========
    print("\n" + "=" * 100)
    print("🔥 전이학습 시작! (Resolution: 800x800, Batch Size: 8, Epochs: 80, LR: 0.00005)")
    print("  🎨 강화 증강: 45° 회전 + Perspective + 조명 변화")
