...
def calculate_metrics(model, dataloader, device, img_size=640, use_tta=False):
    ...
    with torch.no_grad():
        for images, boxes, points in dataloader:
            ...
            # 통합 Loss (Box와 Point 모두 2배 가중치)
            loss = (2.0 * box_loss) + (2.0 * point_loss)
...
def main():
    ...
    best_epoch = 0
    patience_limit = 50  # Early stopping patience (넉넉히 설정)
    patience_counter = 0
    ...
    print("\n" + "=" * 100)
    print("🔥 전체 학습 시작! (Batch Size: 16, Epochs: 300, LR: 0.0001)")
