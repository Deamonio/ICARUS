...
class CupDataset(Dataset):
    def __init__(self, img_dir, label_dir, img_size=640, augment=False):
        ...
        # 증강 설정 - [최종] 0% 모드: 모든 증강 제거, 오직 정답 이미지에만 집중
        if self.augment:
            self.transform = A.Compose([
                A.Resize(img_size, img_size),
                # 모든 증강 제거 - 1px 오차를 잡기 위한 순수 학습
    ...
    def __getitem__(self, idx):
        ...
        # 증강 후 bbox나 keypoint가 사라진 경우 원본 데이터 사용
        if len(transformed['bboxes']) == 0 or len(transformed['keypoints']) == 0:
            # 증강 없이 기본 변환만 적용
            ...
            transformed = basic_transform(image=image, bboxes=[bbox], class_labels=[0], keypoints=keypoint)
...
class YOLOFullTaskModel(nn.Module):
    def __init__(self):
        ...
        # YOLOv8n 가중치 사용 (Stage 1과 동일한 아키텍처)
        model_path = 'yolov8n-pose.pt'
        if not os.path.exists(model_path):
            print(f"⬇️  {model_path} 파일이 없습니다. 자동 다운로드 중...")
            # YOLO 클래스는 파일이 없으면 자동으로 다운로드
        
        base_model = YOLO(model_path).model
        self.feature_extractor = base_model.model[:10] # Backbone
        
        # Feature dimension (YOLOv8n은 256)
        feature_dim = 256
...
def calculate_metrics(model, dataloader, device, img_size=640, use_tta=False):
    ...
    criterion_point = nn.SmoothL1Loss(beta=0.0005)  # 극초정밀 모드 (0.5px 단위)
    ...
    with torch.no_grad():
        for images, boxes, points in dataloader:
            ...
            # Box Loss 계산
            box_loss = criterion_box(p_boxes, boxes)
            point_loss = criterion_point(p_points, points)
            total_box_loss += box_loss.item()
            
            # [박스 굳히기] 통합 Loss (Box=10.0x, Point=50.0x) - 박스 강화 + 점 균형
            loss = (10.0 * box_loss) + (50.0 * point_loss)
...
def plot_training_metrics(history, save_path='training_metrics.png', best_epoch=None):
    ...
    fig.suptitle('Transfer Learning Stage 3 - Precision Training (Box=5.0x, Point=5.0x)', fontsize=16, fontweight='bold')
...
def main():
    ...
    # ========== 데이터셋 로드 ==========
    print("\n📦 데이터셋 로드 중...")
    train_dataset = CupDataset('./dataset/images/train', './dataset/labels/train', augment=False)  # 증강 완전 제거
    ...
    # 최신 Stage 3 가중치를 우선으로 로드
    if os.path.exists('cup_model_best_stage3.pth'):
        model.load_state_dict(torch.load('cup_model_best_stage3.pth'))
        print("🔄 Stage 3 베스트 모델을 불러와 극한 정밀 학습을 시작합니다.")
    elif os.path.exists('cup_model_best_stage2.pth'):
        model.load_state_dict(torch.load('cup_model_best_stage2.pth'))
        print("🔄 Stage 2 베스트 모델을 불러와 정밀 학습을 시작합니다.")
    elif os.path.exists('cup_model_best.pth'):
        model.load_state_dict(torch.load('cup_model_best.pth'))
        print("🔄 Stage 1 베스트 모델을 불러와 학습을 시작합니다.")
    else:
        print("⚠️  기존 가중치를 찾을 수 없습니다. 초기 가중치로 시작합니다.")

    # [최종] 러닝레이트 초미세 사포질 (2e-6 고정)
    optimizer = torch.optim.Adam(model.parameters(), lr=0.000002)  # 2e-6 (미세 수렴)

    # [스케줄러 비활성화] 고정 lr로 미세 조정
    # scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
    #     optimizer, mode='min', factor=0.5, patience=15, verbose=True
    # )
    criterion_box = nn.MSELoss()
    criterion_point = nn.SmoothL1Loss(beta=0.0005)  # beta=0.0005 (0.3px) - 극초정밀 모드
    ...
    csv_filename = f'transfer_stage3_improved_{timestamp}.csv'
    graph_filename = f'transfer_stage3_improved_{timestamp}.png'
    history = []
    start_epoch = 0
    ...
    print(f"  🔧 개선 사항: YOLOv8m + Feature 512 + 강화된 증강 + 500 epochs")
    ...
    patience_counter = 0
    best_model_path = 'cup_model_best_stage3.pth'  # Stage 3 (개선 버전)
    latest_model_path = 'cup_model_latest_stage3.pth'
    epochs = 300  # 초미세 사포질 300 에포크

    # ========== 학습 루프 (Early Stopping 포함) ==========
    print("\n" + "=" * 100)
    print("🔥 전이 학습 3단계 시작! (실전 환경 최적화)")
    print(f"  🤖 모델: YOLOv8m (Feature: 512)")
    ...
    print(f"  🎯 Loss: Box=MSE(3.0x), Point=SmoothL1(beta=0.01, 15.0x)")
    print(f"  🎨 Augmentation: 실전 대비 (CLAHE, Perspective, MotionBlur, GaussNoise)")
    print(f"  🔧 증강: 강화된 ShiftScaleRotate + RandomRotate90 + CoarseDropout")
    print("=" * 100 + "\n")
    ...
    for epoch in range(epochs):
        ...
        for images, boxes, points in train_loader:
            ...
            if use_cuda:
                with torch.amp.autocast('cuda'):
                    ...
                    box_loss = criterion_box(p_boxes, boxes)
                    point_loss = criterion_point(p_points, points)
                    # [박스 굳히기] 학습 루프 내 Loss 계산 부분
                    # 박스 강화(10.0) + 중심점 균형(50.0)
                    loss = (10.0 * box_loss) + (50.0 * point_loss)
        ...
        # Learning Rate Scheduler 업데이트 (비활성화됨)
